...
<%
    ...
    if (productId != null && !productId.trim().isEmpty()) {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("SELECT * FROM tops WHERE id = ?");
...
<script>
    // Function to check if user is logged in and handle adding to cart
    function checkLoginForCart(productId, productName, price, imageUrl, size) {
        var isLoggedIn = <%= session.getAttribute("username") != null ? "true" : "false" %>;
        if (!isLoggedIn) {
            alert("Please login first before adding to the cart.");
    ...
    // Function to check if user is logged in and handle buying
    function checkLoginForBuy(productId, productName, price, imageUrl, size) {
        var isLoggedIn = <%= session.getAttribute("username") != null ? "true" : "false" %>;
        if (!isLoggedIn) {
