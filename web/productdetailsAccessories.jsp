<%-- 
    Document   : productdetailsAccessories
    Created on : 12 Sept 2024, 12:49:14 pm
    Author     : ashle
--%>
    ...
    <header>
        <div class="top-bar">
            ...
            <div class="user-actions">
                ...
                <a href="<%= request.getContextPath() %>/wishlist.jsp"><img src="photos/shopping-cart.png" alt=""/></a>
...
<%
    ...
    if (productId != null && !productId.trim().isEmpty()) {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("SELECT * FROM accessories WHERE id = ?");
                ...
                %>
                    ...
                    <form action="<%= request.getContextPath() %>/addtocart.jsp" method="post">
                        ...
                        <input type="hidden" name="imageUrl" value="<%= rs.getString("image_url") %>">
                         <button onclick="checkLoginForCart('cart', '<%= rs.getString("id") %>', '<%= rs.getString("name") %>', '<%= rs.getDouble("price") %>', '<%= rs.getString("image_url") %>') %>')">Add to Cart</button>
                    ...
                    <form action="<%= request.getContextPath() %>/bill.jsp" method="post">
                        ...
                        <button onclick="checkLoginForBuy('buy', '<%= rs.getString("id") %>', '<%= rs.getString("name") %>', '<%= rs.getDouble("price") %>', '<%= rs.getString("image_url") %>') %>')">Buy Now</button>
                    </form>
                <%
